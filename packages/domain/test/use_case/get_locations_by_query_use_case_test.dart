...
void main() {
  ...
  test('Given correct parameters it invokes every method in usecase and returns locations list', () async {
    ...
    result.fold((l) => throw Exception(l.toString()), (r) => expect(r, [location]));
  });

  test('Given ErrorDetailFatal to WeatherDataSource it returns GetWeatherForSavedLocationFailure.unexpected and invokes every method in use case', () async {
    when(() => mockWeatherDataSource.getLocationByQuery(any())).thenAnswer(
      (_) => TaskEither.left(ErrorDetailFatal()),
    );
